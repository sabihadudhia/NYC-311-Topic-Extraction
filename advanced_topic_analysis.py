...
max_svd_components = min(max(1, n_samples - 1), max(1, n_features - 1))
n_components = min(50, max_svd_components)

if n_components < 1:
    raise ValueError("Unable to apply SVD: not enough samples/features after vectorization.")
...
lda_topics = best_lda.transform(bow_matrix)
...
valid_cluster_range = [cluster for cluster in cluster_range if 1 < cluster < len(corpus)]
if not valid_cluster_range:
    raise ValueError("No valid cluster values in --cluster-range. Each value must be greater than 1 and less than number of documents.")
...
davies_bouldin = davies_bouldin_score(tfidf_reduced, best_labels)
...
n_samples = tfidf_reduced.shape[0]
if n_samples < 3:
    print("  Skipped: not enough samples for t-SNE visualization.")
    tsne_results = None
else:
    # Keep perplexity valid for dataset size and stable for small datasets.
    tsne_perplexity = min(30, max(2, n_samples // 10))
...
analysis_summary = {
    ...
    'KMeans_Davies_Bouldin': davies_bouldin,
